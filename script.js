document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- 0. GLOBAL VARIABLES (Must be declared first) ---
    const widgetInstances = {}; // 🔑 ย้ายมาไว้บนสุด เพื่อให้ทุกฟังก์ชันมองเห็น
    let grid = null;
    let socket = null;
    let db = null;
    let SQL = null;
    ...
    function logToConsole(msg, type='normal') {
        ...
        if (isScrolledToBottom) consoleOut.scrollTop = consoleOut.scrollHeight;
    }
    ...
    try {
        if (typeof io !== 'undefined') {
            ...
            logToConsole("Socket.io not loaded. Backend disabled.", "error");
        }
    } catch(e) { logToConsole(`Socket Init Error: ${e}`, "error"); }
    ...
    } else { logToConsole("SQL.js library missing.", "error"); }
    ...
    const createGridStackItem = (title, type, config, w = 3, h = 2, isLocked = false) => {
        ...
        // กำหนด Attribute ไว้ที่ Element ด้วย (Backup)
    ...
    fileInputLayout.addEventListener('change', (e) => {
        ...
        reader.onload = (ev) => {
            try {
                ...
                // 🔑 1. ลบของเก่าออกให้หมด (true = remove DOM node ด้วย)
                grid.removeAll(true);
                ...
                layoutData.forEach(item => {
                    ...
                    // เพิ่มเข้า Grid (ส่ง w, h, x, y เข้าไปใน options เลย เพื่อความชัวร์)
    ...
    // --- 6. MODAL LOGIC (FIXED WIDGETINSTANCES ERROR) ---
    ...
    let isOverlayClickStarted = false;
    ...
    function closeModal() {
        try {
            if (currentEditingWidget) {
                ...
                // 🔑 เช็ค widgetInstances ที่เราประกาศไว้ด้านบนสุด (Scope ถูกต้องแล้ว)
            ...
            logToConsole("Error saving settings: " + err.message, "error");
        } finally {
    ...
    modalOverlay.addEventListener('mousedown', (e) => {
        if (e.target === modalOverlay) isOverlayClickStarted = true;
        else isOverlayClickStarted = false;
    });

    modalOverlay.addEventListener('mouseup', (e) => {
        if (isOverlayClickStarted && e.target === modalOverlay) closeModal();
        isOverlayClickStarted = false;
    });
    ...
    btnNewDbFile.addEventListener('click', async () => {
        ...
        if ('showSaveFilePicker' in window) {
            try {
                ...
                dbPathBox.classList.add('has-file'); 
            ...
            } catch (err) { if(err.name !== 'AbortError') logToConsole(`Action Cancelled`, "normal"); }
        } else { alert("Browser not supported. Use Auto-Generate."); }
    });
        ...
        if (file) {
            ...
            reader.onload = function() {
                try {
                    ...
                    // Show warning for fallback open as well
                    dbWarningText.style.display = "flex";
    ...
    dbClearBtn.addEventListener('click', (e) => {
        ...
        dbPathBox.classList.remove('has-file'); 
        dbClearBtn.style.display = "none";
        dbWarningText.style.display = "none"; 
        btnConnectDb.innerText = "Start Recording";
        dbInfoText.innerText = "Ready to Record";
        logToConsole("File selection cleared.", "normal");
    });
    ...
    btnConnectDb.addEventListener('click', async () => {
        ...
        if (!isDbRecording) {
            ...
            // 🔑 Hide warning while recording to clean up UI (or keep it if you prefer)
            // But per requirement "Show warning when open file is active", it stays shown in 'open' mode.
            // If Auto/New, it was hidden already.
    ...
    if(consoleResizer) {
        ...
        const doResize = (y) => {
            ...
            let h = document.body.clientHeight - y;
            if (h < 50) { 
        ...
        consoleResizer.addEventListener('mousedown', (e) => { e.preventDefault(); startResize(e.clientY); });
